...
function mapClientToDb(client){
  return {
    ...
    documento:String(client.documento||'').replace(/\D/g,'').trim() || null,
...
async function loadClientsFromCloud(){
  ...
  const { data, error }=await supabaseClient
    .from('clientes')
    .select('id,nome,telefone,documento,plano,valor,pagamento,vencimento,tags,payment_history,parcelado,parcelas_total,parcelas_pagas,juros_ativo,juros_percentual,created_at')
...
function renderGestorToday(){
  ...
  if(resolve){
    resolve.textContent=lateRows.length?'Ver inadimplentes':next7.length?'Ver clientes que vencem':'Tudo em dia';
    resolve.disabled=!lateRows.length && !next7.length;
    resolve.dataset.target=lateRows.length?'delinquencySection':'clientsSection';
  }
}
...
function normalizePlanLabel(value){
  ...
  if(!raw) return '';
  if(raw==='mensal' || raw.includes('mensal')) return 'Mensal';
  if(raw==='semestral' || raw.includes('semestral') || raw.includes('6 mes')) return 'Semestral';
  if(raw==='anual' || raw.includes('anual') || raw.includes('12 mes')) return 'Anual';
  return 'Outros';
}
function renderDashboardInsights(){
  const total=clients.length;
  const late=clients.filter(c=>statusOf(c).key==='late').length;
  const active=Math.max(0,total-late);
  const activePct=total?Math.round(active/total*100):0;
  const latePct=total?100-activePct:0;
  const set=(id,value)=>{const el=document.getElementById(id);if(el)el.textContent=value;};
  set('statusTotal',String(total));
  set('statusActive',`${active} (${activePct}%)`);
  set('statusLate',`${late} (${latePct}%)`);
  const donut=document.getElementById('statusDonut');
  if(donut){
    donut.style.background=total
      ? `conic-gradient(#35d6c2 0 ${activePct}%, #ff5d78 ${activePct}% 100%)`
      : 'conic-gradient(rgba(255,255,255,.10) 0 100%)';
  }

  const counts={Mensal:0,Semestral:0,Anual:0};
  clients.forEach(c=>{
    const label=normalizePlanLabel(c.plano);
    if(Object.prototype.hasOwnProperty.call(counts,label)) counts[label]++;
  });
  const classifiedTotal=counts.Mensal+counts.Semestral+counts.Anual;
  const list=document.getElementById('plansUsageList');
  if(list){
    const rows=['Mensal','Semestral','Anual'];
    list.innerHTML=rows.map(label=>{
      const count=counts[label]||0;
      const pct=classifiedTotal?Math.round(count/classifiedTotal*100):0;
      return `<div class="plan-usage-row"><div class="plan-usage-name"><strong>${label}</strong><span>${count} cliente${count===1?'':'s'}</span></div><div class="plan-usage-bar"><i style="width:${pct}%"></i></div><div class="plan-usage-pct">${pct}%</div></div>`;
    }).join('');
  }
}
...
function renderList(){
  ...
  let filtered = clients.filter(c=>{
    ...
    const matchesSearch = !search ||
      ...
      (c.plano||'').toLowerCase().includes(search) ||
...
window.gpFormatCpfCnpj=formatCpfCnpj;
document.getElementById('f-documento')?.addEventListener('input',e=>{e.target.value=formatCpfCnpj(e.target.value);});
...
function openForm(client){
  ...
  document.getElementById('f-documento').value = client ? formatCpfCnpj(client.documento) : '';
  document.getElementById('f-plano').value = client ? (normalizePlanLabel(client.plano)==='Outros' ? '' : normalizePlanLabel(client.plano)) : '';
...
document.getElementById('quickAddClient')?.addEventListener('click',()=>{
  document.getElementById('clientsSection')?.scrollIntoView({behavior:'smooth',block:'start'});
  setTimeout(()=>openForm(null),220);
});
document.getElementById('quickDelinquency')?.addEventListener('click',()=>{
  document.getElementById('delinquencySection')?.scrollIntoView({behavior:'smooth',block:'start'});
});
document.getElementById('quickReports')?.addEventListener('click',()=>{
...
document.getElementById('quickSettings')?.addEventListener('click',()=>{
  const btn=document.getElementById('companyNavBtn');
  if(btn) btn.click();
});
...
document.getElementById('quickFilters').addEventListener('click',e=>{
  const btn=e.target.closest('.quick-filter[data-filter]');
  if(!btn)return;
  const filter=document.getElementById('filter');
  const search=document.getElementById('search');
  if(filter) filter.value=btn.dataset.filter || 'todos';
...
function openClientProfile(id){
  ...
  document.getElementById('clientProfileDetails').innerHTML=`
    ...
    <div class="client-profile-line"><span>CPF / CNPJ</span><b>${gpEsc(formatCpfCnpj(c.documento)||'Não informado')}</b></div>
